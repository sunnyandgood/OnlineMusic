            ...
            if(confirm('是否删除')){
                $(document).ready(function () {
                    $.ajax({
                        ...
                        url:"${ctx}/ClicksServlet?state=deleteById&clickId=" + row['click_id'],//传输路径
        ...
        if (confirm("全部删除？")){
            ...
            $(document).ready(function () {
                $.ajax({
                    type:"post",//提交方式
                    dataType: "json",//预期服务器返回的数据类型
                    url:"${ctx}/ClicksServlet?state=deleteByIds&clickIds=" + ids,//传输路径
                    success:function (msg) {
                        if (msg.code==200){
                            layer.msg(msg.message);
                            $('#table').bootstrapTable('refresh');
    ...
    function addToExcel() {
        $(document).ready(function () {
            $.ajax({
                type:"post",//提交方式
                dataType: "json",//预期服务器返回的数据类型
                url:"${ctx}/ClicksServlet?state=addToExcel",//传输路径
                success:function (msg) {
                    if (msg.code==200){
                        layer.msg(msg.message);
                        $('#table').bootstrapTable('refresh');
