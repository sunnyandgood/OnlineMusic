...
<script>
            ...
            if(confirm('是否删除')){
                ...
                $(document).ready(function () {
                    $.ajax({
                        ...
                        url:"${ctx}/SongTypeServlet?state=deleteById&typeId=" + row['type_id'],//传输路径
        ...
        if (confirm("全部删除？")){
            ...
            $(document).ready(function () {
                $.ajax({
                    type:"post",//提交方式
                    dataType: "json",//预期服务器返回的数据类型
                    url:"${ctx}/SongTypeServlet?state=deleteByIds&typeIds=" + ids,//传输路径
                    success:function (msg) {
                        if (msg.code==200){
                            layer.msg(msg.message);
                            $('#table').bootstrapTable('refresh');
    ...
    function add(){
        layer.open({
            type: 2,
            area: ['800px', '450px'],
            content: '${ctx}/page/admin/add/songtype_add_jsp' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    }
    ...
    function addToExcel() {
        $(document).ready(function () {
            $.ajax({
                type:"post",//提交方式
                dataType: "json",//预期服务器返回的数据类型
                url:"${ctx}/SongTypeServlet?state=addToExcel",//传输路径
                success:function (msg) {
                    if (msg.code==200){
                        layer.msg(msg.message);
                        $('#table').bootstrapTable('refresh');
