...
<script>
            ...
            if(confirm('是否删除')){
                ...
                $(document).ready(function () {
                    $.ajax({
                        ...
                        url:"${ctx}/UserServlet?state=deleteById&userId=" + row['user_id'],//传输路径
        ...
        if (confirm("全部删除？")){
            ...
            $(document).ready(function () {
                $.ajax({
                    type:"post",//提交方式
                    dataType: "json",//预期服务器返回的数据类型
                    url:"${ctx}/UserServlet?state=deleteByIds&userIds=" + ids,//传输路径
                    success:function (msg) {
                        if (msg.code==200){
                            layer.msg(msg.message);
                            $('#table').bootstrapTable('refresh');
    ...
    function addFromExcel() {
        layer.open({
            type: 2,
            area: ['500px', '350px'],
            content: ['${ctx}/page/admin/upload/user_upload_jsp', 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    }
    ...
    function addToExcel() {
        $(document).ready(function () {
            $.ajax({
                type:"post",//提交方式
                dataType: "json",//预期服务器返回的数据类型
                url:"${ctx}/UserServlet?state=addToExcel",//传输路径
                success:function (msg) {
                    if (msg.code==200){
                        layer.msg(msg.message);
                        $('#table').bootstrapTable('refresh');
