...
<script>
    ...
    $('#table').bootstrapTable({
        //获取数据的Servlet地址
        url:'${ctx}/SongTypeServlet?state=listAll',
        columns: [{
            checkbox:true
        }, {
            title: '序号',
    ...
    window.caocuoEvents = {
        'click .update': function (e, value, row) {
            layer.open({
                type: 2,
                area: ['800px', '500px'],
                content: '${ctx}/SongTypeServlet?state=selectById&typeId=' + row['type_id'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
            });
        },
        'click .remove': function (e, value, row) {
            if(confirm('是否删除')){
                $.post('${ctx}/SongTypeServlet?state=deleteById&typeId=' + row['type_id'],function(r){
    ...
    function removeAll() {
        if (confirm("全部删除？")){
            ...
            $.post('${ctx}/SongTypeServlet?state=deleteByIds&typeIds=' + ids,function (r) {
                $('#table').bootstrapTable('refresh');
            });
        }
    }
    ...
    function add(){
        layer.open({
            type: 2,
            area: ['800px', '500px'],
            content: '${ctx}/admin/add/songtype_add.jsp' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    }

    function addToExcel() {
        $.post('${ctx}/SongTypeServlet?state=addToExcel',function (r) {
            // layer.msg(r.message);
            layer.msg("导出成功！");
        });
    }
