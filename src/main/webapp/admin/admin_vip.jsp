...
<script>
    ...
    window.caocuoEvents = {
        ...
        'click .remove': function (e, value, row) {
            if(confirm('是否删除')){
                $.post('${ctx}/VipServlet?state=deleteById&vipId=' + row['vip_id'],function(r){
    ...
    function removeAll() {
        if (confirm("全部删除？")){
            ...
            $.post('${ctx}/VipServlet?state=deleteByIds&vipIds=' + ids,function (r) {
                $('#table').bootstrapTable('refresh');
            });
        }
    }
    ...
    function add(){
        layer.open({
            type: 2,
            area: ['800px', '500px'],
            content: '${ctx}/admin/add/vip_add.jsp' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    }

    function addToExcel() {
        $.post('${ctx}/VipServlet?state=addToExcel',function (r) {
            // layer.msg(r.message);
            layer.msg("导出成功！");
        });
    }
