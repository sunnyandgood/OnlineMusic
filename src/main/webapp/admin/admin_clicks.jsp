...
<script>
    ...
    $('#table').bootstrapTable({
        //获取数据的Servlet地址
        url:'${ctx}/ClicksServlet?state=listAll',
        columns: [{
            checkbox:true
        }, {
            title: '序号',
    ...
    window.caocuoEvents = {
        'click .remove': function (e, value, row) {
            if(confirm('是否删除')){
                $.post('${ctx}/ClicksServlet?state=deleteById&clickId=' + row['click_id'],function(r){
    ...
    function removeAll() {
        if (confirm("全部删除？")){
            ...
            $.post('${ctx}/ClicksServlet?state=deleteByIds&clickIds=' + ids,function (r) {
                $('#table').bootstrapTable('refresh');
            });
        }
    }
    ...
    function addToExcel() {
        $.post('${ctx}/ClicksServlet?state=addToExcel',function (r) {
            layer.msg("导出成功！");
        });
    }
</script>
