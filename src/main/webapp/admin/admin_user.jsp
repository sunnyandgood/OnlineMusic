...
<script>
    ...
    $('#table').bootstrapTable({
        //获取数据的Servlet地址
        url:'${ctx}/UserServlet?state=listAll',
        columns: [{
            checkbox:true
        }, {
            title: '序号',
    ...
    window.caocuoEvents = {
        'click .update': function (e, value, row) {
            layer.open({
                type: 2,
                area: ['800px', '500px'],
                content: '${ctx}/UserServlet?state=selectById&userId=' + row['user_id'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
            });
        },
        'click .remove': function (e, value, row) {
            if(confirm('是否删除')){
                $.post('${ctx}/UserServlet?state=deleteById&userId=' + row['user_id'],function(r){
    ...
    function removeAll() {
        if (confirm("全部删除？")){
            ...
            $.post('${ctx}/UserServlet?state=deleteByIds&userIds=' + ids,function (r) {
                $('#table').bootstrapTable('refresh');
            });
        }
    }
    ...
    function add(){
        layer.open({
            type: 2,
            area: ['800px', '500px'],
            content: '${ctx}/UserServlet?state=selectVipAndSongType' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    }
    ...
    function addFromExcel() {
        layer.open({
            type: 2,
            area: ['500px', '350px'],
            content: ['${ctx}/admin/upload/user_upload.jsp', 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    }

    function addToExcel() {
        $.post('${ctx}/UserServlet?state=addToExcel',function (r) {
            layer.msg("导出成功！");
        });
    }
</script>
